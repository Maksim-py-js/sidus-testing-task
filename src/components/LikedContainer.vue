...
<script lang="ts">
    ...
    export default {
        components: { PopupContainer, Button, Text3, Heart },
        methods: {
            likePhoto() {
                let container : any = document.getElementById("popup_2");
                let newLike = document.createElement("div");

                newLike.className = "heart";
                newLike.style.left = Math.floor(Math.random() * (container.offsetWidth - 60 + 1)) + 'px';
                newLike.style.top = Math.floor(Math.random() * (container.offsetHeight - 146 + 1)) + 'px';
                newLike.innerHTML = 
